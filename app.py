...
def _spreadsheet_id() -> str:
    v = _get_secret("SPREADSHEET_ID", "GOOGLE_SHEET_ID")
    if not v:
        st.error("ยังไม่ได้ตั้งค่า SPREADSHEET_ID ใน Streamlit Secrets\n"
...
def _drive_folder_id() -> str:
    return _get_secret("DRIVE_FOLDER_ID", "GOOGLE_DRIVE_FOLDER_ID")
...
def _get_credentials():
    ...
    try:
        ...
        # แก้ private_key ที่อาจมี \n เป็นข้อความแทน newline จริง
...
def _get_or_create_site_folder(site_name: str) -> str:
    ...
    query = (
        ...
        f"and '{_drive_folder_id()}' in parents and trashed = false"
    )
...
def _fetch_drive_image_bytes(url: str):
    """ดาวน์โหลดรูปจาก URL ตรงๆ (Cloudinary, Drive, หรือ URL ทั่วไป)"""
    if not url:
        return None
    try:
        ...
        print(f"[เตือน] ดาวน์โหลดรูปไม่สำเร็จ ({url}): {e}")
        return None
...
def upload_report_to_drive(site_name: str, doc_buf: io.BytesIO) -> str:
    """ไม่ใช้ Drive แล้ว — คืนค่าว่างเพื่อให้ดาวน์โหลดตรงแทน"""
    return ""
...
def page_entry():
    ...
    with col_exp1:
        if st.button("✅ ยืนยันไซต์งาน", type="primary", use_container_width=True):
            df3 = load_history()
    ...
    with col_exp2:
        ...
        with st.spinner("กำลังสร้างรายงาน..."):
            doc_buf = build_word_report(site_name, site_data, ms2, me2)
        st.download_button(
